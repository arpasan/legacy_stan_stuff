...
We fetch the dataset from [here](https://github.com/CapitalOneRecruiting/DS). This remaster ships a **20,000-row** extract (`data/transactions_sample.jsonl`) so the notebook is reproducible from the repo; drop the full `transactions.txt` into `data/` to rerun on the complete Capital One dump.
...
Now, leave-one-out predictive probabilities via `rstanarm::loo_predict` (wrapper around `E_loo` that avoids the Inf/NA/constant-tail GPD failures from calling `E_loo` on the raw epred matrix with `-log_lik`).
...
<em>Revision 2026-07-18c — Model 3 chart source cleaned to match this document’s style.</em>

One more Bayesian attempt — not a model zoo. Model 1’s sklearn $\ell_2$ logit is the bar to clear (**70.48%** holdout accuracy on this balanced extract). Model 2’s flat `stan_glm` logit is more honest about uncertainty but lands nearer 60% under LOO. Here we stay fully Bayesian and try to beat Model 1 on the **same frame and split**.
...
Same 20% holdout as Model 1: holdout accuracy moves from **70.48%** (sklearn $\ell_2$ logit) to **~71.4%** under the horseshoe posterior mean, with a better Brier score as well. GPyTorch variational GPs were tried on this extract and did not clear the Model 1 bar, so the shipped attempt is this single CmdStanPy horseshoe logit — one Bayesian model, one predictive chart, no foil stack.
