...
Time series models are sensitive to missingness, so check the country-day panel.

```{r missingness, fig.height=6, fig.width=9}
vis_miss(panel, warn_large_data = FALSE)
```

The data are sparse on some days for some mobility series, so exploratory charts below aggregate to weeks (and one monthly mobility panel). Day-to-day jitter is likely too granular for the DiD story anyway.
