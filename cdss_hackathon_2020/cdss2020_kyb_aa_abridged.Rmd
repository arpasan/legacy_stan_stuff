...
Time series models are sensitive to missingness. On the raw **region-day** mobility extract, several series have gaps; after collapsing to **country-day** means and joining ECDC, the modeling panel is complete (so a `vis_miss` heatmap would just be a solid “present” block — useless). Compare the two grains:

```{r missingness}
miss_region <- miss_var_summary(mobility_region) |>
  filter(n_miss > 0) |>
  arrange(desc(pct_miss))

miss_panel <- miss_var_summary(panel) |>
  arrange(desc(pct_miss))

knitr::kable(
  miss_region,
  digits = 2,
  caption = "Variables with any missingness in region-day mobility (before country-day collapse)"
)

knitr::kable(
  miss_panel,
  digits = 2,
  caption = "Missingness in the country-day modeling panel (should be all zeros)"
)
```
