...
Time series models are sensitive to missingness. Compare percent missing by variable in the raw region-day mobility extract vs the country-day modeling panel (full names on the axis — not a `naniar` heatmap).

```{r missingness, fig.height=5.5, fig.width=8}
miss_compare <- bind_rows(
  miss_var_summary(mobility_region) |> mutate(grain = "region-day mobility"),
  miss_var_summary(panel) |> mutate(grain = "country-day panel")
) |>
  mutate(
    variable = factor(variable, levels = rev(sort(unique(variable)))),
    grain = factor(grain, levels = c("region-day mobility", "country-day panel"))
  )

ggplot(miss_compare, aes(pct_miss, variable)) +
  geom_col(fill = "#4a4a4a", width = 0.7) +
  facet_wrap(~grain, nrow = 1) +
  scale_x_continuous(limits = c(0, NA), expand = expansion(mult = c(0, 0.08))) +
  labs(
    title = "Percent missing by variable",
    x = "% missing",
    y = NULL
  ) +
...
knitr::kable(
  miss_compare |>
    select(grain, variable, n_miss, pct_miss) |>
    arrange(grain, desc(pct_miss)),
  digits = 2,
  caption = "Same missingness numbers in table form"
)
```

Day-to-day jitter is still noisy for the narrative charts, so the exploratory plots below aggregate to weeks.
